 ...
 * Ensures footer prints on every page and handles remote logos.
...
export async function buildQuotePdf(data) {
  ...
  // Prepare logoBuffer if garage.logo is a remote URL
  let logoBuffer;
  if (data.garage && data.garage.logo && /^https?:\/\//.test(data.garage.logo)) {
    try {
  ...
  // Ensure footer on first page and subsequent pages
  addFooter(doc);
  doc.on('pageAdded', () => addFooter(doc));

  // Header with dynamic logo and company name
