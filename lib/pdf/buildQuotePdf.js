 ...
 * Ensures fallback to default logo if remote fetch fails and prints footer on every page.
...
export async function buildQuotePdf(data) {
  ...
  // Attempt to fetch a remote logo if provided
  let logoBuffer;
  if (data.garage && data.garage.logo && data.garage.logo.includes('://')) {
    try {
