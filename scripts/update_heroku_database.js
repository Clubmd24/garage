...
async function updateHerokuDatabase() {
  ...
  try {
    ...
    // Also set it as a separate variable for backup
    console.log('📝 Setting NEW_DATABASE_URL as backup...');
    execSync(`heroku config:set NEW_DATABASE_URL="${NEW_DATABASE_URL}"`, { stdio: 'inherit' });
