  ...
  if (req.method === 'GET') {
    try {
      const [[task]] = await pool.query(
        'SELECT t.*, u.username AS assignee FROM dev_tasks t LEFT JOIN users u ON t.assignee_id = u.id WHERE t.id = ?',
        [id]
      );
  ...
  if (req.method === 'PUT') {
    ...
    try {
      ...
      const [result] = await pool.query(
        'UPDATE dev_tasks SET title = ?, description = ?, status = ?, assignee_id = ?, due_date = ? WHERE id = ?',
